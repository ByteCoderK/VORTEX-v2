...
logger = logging.getLogger("vortex.weather")
...
def live_weather(city):
    ...
    if response.status_code == 200:
        logger.info("Weather fetch successful for city=%s", city)
        ...
        logger.warning("Weather fetch failed for city=%s status=%s", city, response.status_code)
